...
9. [Public Network Incentive Layer](#9-public-network-incentive-layer) — Public network coordination economics (private deployments use zero tokens)
...
7. **Longevity** — designed to operate for 1,000 years without institutional dependency (aspirational design constraint; no software system has achieved this). Achievable because validation proofs are mathematical relationships, not application state — they survive the death of every system that created them and can be verified on any future medium
8. **Accessibility** — run on devices from a $4 microcontroller (via gateway-delegated signing, Profile C) to a datacenter

For enterprise, government, and defense deployments that require private operation with zero token involvement, see Section 10.6 — Private Networks and Network Publication. These deployments use the same cryptographic protocol and data structure as the public network.
...
No internet. No central server. No cloud dependency. No token. No fee.
...
> **Scope: Public permissionless network only.** This section describes the incentive mechanism for coordinating a public network where participants have no pre-existing trust relationship. Enterprise, government, and defense deployments operate as private networks (Section 10.6) with zero token involvement. Private networks use the same cryptographic protocol (Layer 1) and the same data structure (DAM) without any economic layer. Tokens are not required for validation, signing, verification, or any cryptographic operation.

> **Full specification:** The complete public network economic model — including supply mechanics, distribution, governance economics, storage markets, anti-centralization mechanisms, and regulatory analysis — is specified in a companion economic specification (separate document, versioned independently). This section provides a protocol-level summary.
...
3. **Storage delegation** — nodes that cannot store records long-term pay storage-specialized nodes to hold them. The delegating node always retains its signed record header (see companion economic specification, Section 4).

4. **Governance participation** — token holders participate in protocol governance through conviction voting (Section 10.3), subject to anti-centralization constraints detailed in the companion economic specification.

### 9.2 Design Principles

The token economic model is guided by four principles:

- **Conservation over inflation** — the protocol targets a fixed-supply model where tokens circulate between producers (witnesses) and consumers (record submitters) rather than being continuously minted. See the companion economic specification for supply mechanics.
...
- **Utility, not speculation** — the token launches with network utility, not as an investment vehicle. No ICO, no pre-sale (see companion economic specification for launch strategy and regulatory analysis).
...
- **Storage delegation** — nodes delegate record storage to storage-specialized nodes in exchange for tokens (see companion economic specification)

For the complete economic specification — including supply model, distribution schedule, anti-centralization mechanisms (diminishing returns, quadratic governance, trust-weighted committee selection), storage delegation markets, Sybil cost analysis, and securities law analysis — refer to the companion economic specification.
...
> **Note:** The companion economic specification specifies additional governance mechanisms — including trust-weighted random committee selection, identity-based voting caps, and anti-pooling measures — that complement the conviction voting model described here.
...
where τ = 7 days, and the 5% cap is per-identity (companion economic specification, Section 6.5)
```

An entity staking 10,000 tokens has √10 ≈ 3.16× the influence of an entity staking 1,000 tokens, not 10×. The conviction curve then weights by lock duration (reaching ~98.6% at 30 days), and the hard cap ensures no single identity exceeds 5% of total governance weight regardless of stake size. See the companion economic specification for extended analysis of pool centralization risks and additional anti-centralization mechanisms.
...
The companion economic specification (v0.3.3, Section 15) provides detailed analysis of the economic dynamics of this transition, including token demand modeling, anti-gaming mechanisms, and the long-term implications of dual-direction network growth.
...
Token incentives during this phase are elevated — early validators earn disproportionate rewards to compensate for the network's low utility. See the companion economic specification for distribution schedule and bootstrap economics.
...
> **Scope note:** The token economics described in this section apply exclusively to the public permissionless network. Private network deployments (Section 10.6) — enterprise, government, defense — have no token involvement. Layer 1 validation is always free, always offline, always sovereign. No network, no tokens, no fees.

**The question:** Section 3.5 shows that any device can validate locally for free. Section 9 describes token staking for public network witnessing. For participants on the public network without tokens, who pays for their records to be witnessed?

**Solution: Tiered Economic Model**

**Layer 1: Always free. No exceptions.**

Local validation — generating a keypair, hashing content, signing with a private key, storing on local DAG — costs zero tokens. This is a cryptographic operation that runs on the device's own hardware. It is free because it consumes no network resources. Private network deployments operate entirely at this level with organizational witnessing — no token layer exists.
...
A percentage of the Community/Governance token allocation (20% of genesis supply — see companion economic specification, Section 5.1) funds a **public witnessing service** — anchor nodes that attest to free-tier records. This creates a baseline level of network attestation for all participants, regardless of economic status.
...
Nodes that contribute resources (relay bandwidth, storage, compute) earn ELA tokens. The teenager's phone, by relaying other users' records, earns enough tokens to request priority witnessing if she ever needs it. The protocol pays its participants. See the companion economic specification for the complete tiered economic model, storage delegation markets, and earn-by-participation mechanics.
...
- **Node trust score T(n)** — a continuous value in [0, 1] representing a node's accumulated reputation, defined as T(n) = 1 - 1/(1 + attestation_count) where attestation_count is the number of honest attestations by that node (see companion economic specification, Section 6.2). Node trust is earned through sustained honest behavior and cannot be purchased.
- **Witness weight w(n)** — the trust weight of node n's attestation, derived from its node trust score, stake, and age

> **Two levels of trust:** The protocol distinguishes between **node trust** (a node's reputation, earned over time) and **record trust** (network confidence in a specific record, computed from its witness set). Node trust feeds into record trust — a record witnessed by high-trust nodes accumulates record trust faster. The companion economic specification (Section 6.2) specifies the node-level formula; this section specifies the record-level formula.
...
> **Scope: Public permissionless network only.** This section applies to the public permissionless network. Private deployments have no token supply.

> **Full specification:** The complete token supply model — including supply mechanics, minting schedule, burn mechanisms, and conservation economics — is specified in the companion economic specification. This section addresses only the adversarial implications.

The public network's token supply model is designed as a **conservation system** — tokens circulate between producers (witnesses) and consumers (record submitters) rather than being continuously minted. This design choice has specific security implications:
...
See the companion economic specification for the complete supply model, distribution schedule, and economic equilibrium analysis.
...
A witness that carefully evaluates records before attesting — checking for rate limit compliance, duplicate content, identity trust, and causal consistency — maintains high reputation and earns more. Selective, honest witnessing is the Nash equilibrium. See the companion economic specification for the complete witness incentive model and reward mechanics.
...
> **Full analysis in companion economic specification.** The regulatory classification of the ELA token — including Howey test analysis, MiCA compliance strategy, utility-first design rationale, and jurisdictional considerations — is specified in the companion economic specification. This section notes only the protocol-level design choices that support regulatory compliance:
...
The protocol's design prioritizes genuine utility over speculative value. See the companion economic specification for the complete regulatory analysis.

Private deployments (Section 10.6) are not subject to token regulatory considerations as they do not involve tokens. The regulatory analysis applies exclusively to the public permissionless network.
...
The vesting period is tied to the publication rate limit — the longer the publication takes, the longer the token acquisition is spread. This prevents an entity from acquiring all tokens upfront and sitting on them. Token markets can absorb gradual demand over years. They cannot absorb a dominant entity purchasing 40% of the circulating supply in a week.
...
The economic impact of cognitive checkpoints on the public network's token economy is specified in the companion economic specification (Section 4.6).
...
- The Rosetta Stone principle ensures this proof is readable in 1,000 years
- This is the "I was here first" guarantee — simple, transparent, eternal
...
| **ELA**                           | Working name for the public network's utility token. Not required for private network deployments (Section 10.6)             |
...
**Document Hash (SHA-256, v0.6.0):** *computed after final edit*
...
**OpenTimestamps Proof:** `ELARA-PROTOCOL-WHITEPAPER.v0.6.0.md.ots`
...
The `.ots` proof file distributed alongside this document contains a Bitcoin-anchored timestamp proving this document existed at the time of publication. Verify with:

```
ots verify ELARA-PROTOCOL-WHITEPAPER.v0.6.0.pdf.ots
```

Requires the [OpenTimestamps client](https://opentimestamps.org/).

### Elara Protocol (Self-Validating)

This document is signed using the Elara Protocol itself — the same cryptographic system described in this whitepaper. The `.elara.proof` file contains an Ed25519-signed validation record with:

- **SHA-512 file hash** — cryptographic fingerprint of this exact PDF
- **Author identity** — Ed25519 public key of the document signer
- **Timestamp** — device-clock time of signing
- **Chain position** — this proof is linked to the author's previous validation record, forming an unbroken hash chain

Verify with:

```
elara verify ELARA-PROTOCOL-WHITEPAPER.v0.6.0.pdf
```

Or load the `.elara.proof` file at [elara-validate.pages.dev](https://elara-validate.pages.dev) for browser-based verification.
